...
let _SQL = null
let _rawDb = null
let _dirty = false // true when in-memory DB has unsaved-to-Blob writes
...
// Called at the END of every POST request — awaited before the
// response is returned so the Lambda doesn't freeze mid-upload.
export async function syncDb() {
  if (!_dirty || !_rawDb) return
  _dirty = false
  const buf = Buffer.from(_rawDb.export())
  await saveToBlob(buf)
}
...
export async function getDb() {
  ...
  if (fs.existsSync(DB_PATH)) {
    // Same instance already wrote the DB — reuse it
    _rawDb = new _SQL.Database(fs.readFileSync(DB_PATH))
  } else {
    // Cold start: pull the latest DB from Blob
    const blobData = await loadFromBlob()
    if (blobData) {
      _rawDb = new _SQL.Database(blobData)
      fs.writeFileSync(DB_PATH, blobData)
    } else {
      _rawDb = new _SQL.Database() // first-ever deployment
    }
  }
